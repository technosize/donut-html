...
class DonutModelPLModule(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        #added
        with open(r'./output.html', 'a', encoding="utf8") as output_file:
            output_file.write('<style>#box {display: inline-block; margin-top: 1px;}table, th, td {border: 1px solid;width: 100px;height: 10px}</style>')
            ...
            for pred, answer in zip(preds, answers):
                ...
                if self.config.get("verbose", False) and len(scores) == 1:
                    ...
                    output_file.write('<div id="box">GROUND TRUTH'  + BeautifulSoup(answer).prettify() + '</div><div id="box">PREDICTION' + BeautifulSoup(pred).prettify() +  '</div><br>')
